...
🚨 CRITICAL: 반드시 아래 JSON 형식으로만 응답해주세요. 다른 텍스트는 포함하지 마세요.
🚨 CRITICAL: advanced_vocabulary 필드는 반드시 포함되어야 하며, 위에 제공된 "{text}" 텍스트에서 실제로 사용된 어휘만을 추출해야 합니다.
🚨 CRITICAL: "{text}" 텍스트를 다시 한번 확인하고, 그 텍스트에서 사용된 고급 어휘 5개를 정확히 추출하세요.
...
어휘 분석 가이드라인 (필수):
1. 반드시 제공된 글에서 실제로 사용된 어휘만을 선택해야 합니다
2. 글에서 사용된 어휘 중 상위 percentile에 해당하는 수준의 고급 어휘를 정확히 5개 선택
3. 각 어휘의 기본형(원형)을 word 필드에 입력 (예: "사용하다" → "사용")
4. 품사를 pos 필드에 입력 (명사, 동사, 형용사, 부사 등)
5. 해당 어휘의 정의를 definition 필드에 간결하게 설명 (30자 이내)
6. percentile이 높을수록 더 고급 어휘를 선택 (예: 상위 15%면 대학생 이상 수준의 어휘)
7. advanced_vocabulary 필드는 반드시 배열 형태로 제공해야 하며, 5개의 어휘 객체를 포함해야 합니다
8. 절대로 글에 없는 어휘를 만들어내거나 추측하지 마세요
...
- 제공된 텍스트에서 실제로 사용된 어휘만을 선택하세요
- 상위 percentile 수준에 해당하는 고급 어휘 5개를 정확히 추출하세요
- 각 어휘는 반드시 제공된 텍스트에 존재해야 합니다
- 이 필드가 없거나 잘못되면 분석이 실패합니다
